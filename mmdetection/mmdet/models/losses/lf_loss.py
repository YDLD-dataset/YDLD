...
def py_lf_focal_loss_with_prob(pred,
                            target,
                            weight=None,
                            gamma=2.0,
                            alpha=0.25,
...
def py_lf_sigmoid_focal_loss(pred,
                          target,
                          weight=None,
                          gamma=2.0,
                          alpha=0.25,
    ...
    loss = F.binary_cross_entropy_with_logits(
        pred, target, reduction='none') * focal_weight
    if psi is not None:
        lf_weight = (eta - psi) / ((1 - pred_sigmoid) + eps)
        loss = loss * (target + (1 - target) * lf_weight)
...
class LFLoss(nn.Module):
    ...
    def forward(self,
        ...
        if self.activated:
            calculate_loss_func = py_lf_focal_loss_with_prob
        else:
            num_classes = pred.size(1)
            target = F.one_hot(target, num_classes=num_classes + 1)
            target = target[:, :num_classes]
            calculate_loss_func = py_lf_sigmoid_focal_loss
